...
function index(){
	    ...
	    //if user is an admin
        elseif($hasAnyRole('admin')){
        	$id=$roles()->pluck('name')->toArray();
        	$notifications = DB::table('notifications')
		        ->where('notify','=',$id)
		        ->where('notif','!=','Your Application has been Updated!')
        		->paginate(5);
	...
	function appFormPDF(Request $request,$user2){
		$pdf = \App::make('dompdf.wrapper');
		$pdf->loadHTML($this->appFormPDF_convert_to_html($user2));
		return $pdf->stream();
		}

	//this is the html content of the PDF generated for: Application_Form
	public function appFormPDF_convert_to_html($user2){
		/* --------------------  ----  -------------------- */
		/* --------------------  SQLS  -------------------- */
		/* --------------------  ----  -------------------- */
	        ...
	        $profiles = User::find($user)->profiles;
						...
						<tr>
							...
							<td rowspan="2" style="" width="15%">
								...
								if (empty($profiles->photo)) {
									...
									$output .='
									<img src="../public/uploads/profilepicture/'.$profiles->photo.'" style="border:1px solid; width:6rem; height:6rem; " >
									';
								}
							...
							<!-- --------------------  fullname  -------------------- -->
								<li>
									...
									<div class="row" style="border-bottom: 2px solid;">
										'.$profiles->lastName.', '.$profiles->firstName.' '.$profiles->middleName.'
									</div>
								</li>
								...
								<li>
									<table width="100%">
										<tr>
											...
											<td style="border-bottom: 2px solid;">
												'.date("F j, Y", strtotime($profiles->bday)).'
											</td>
										</tr>
									</table>	
								</li>
	...
	function addReqtPDF(Request $request,$user2){
		$pdf = \App::make('dompdf.wrapper');
		$pdf->loadHTML($this->addReqt_convert_to_html($user2));
		return $pdf->stream();
		}

	//this is the html content of the PDF generated for: ETEEAP_Additional_Requirements
	function addReqt_convert_to_html($user2){
		/* --------------------  ----  -------------------- */
		/* --------------------  SQLS  -------------------- */
		/* --------------------  ----  -------------------- */
	        //gets the user's id number
	...
	function narrRepPDF(Request $request,$user2){
		$pdf = \App::make('dompdf.wrapper');
		$pdf->loadHTML($this->narrRep_convert_to_html($user2));
		return $pdf->stream();
		}

	//this is the html content of the PDF generated for: Narrtive_Report
	function narrRep_convert_to_html($user2){
		...
		$output ='
			<!-- --------------------  body  -------------------- -->
				<div style="padding: 6rem 2rem 0rem 2rem; font-size:11pts; font-family: Arial;">
						<p style="font-size: 9pts;" align="right">
							E003 - ETEEAP
						</p>
						<p style="font-weight: bold; font-size: 9pts;" align="center">
	...
	function cvPDF(Request $request,$user2){
		$pdf = \App::make('dompdf.wrapper');
		$pdf->loadHTML($this->cv_convert_to_html($user2));
		return $pdf->stream();
		}

	//this is the html content of the PDF generated for: Curriculum_Vitae
	function cv_convert_to_html($user2){
			        	...
			        	<div class="body" style="page-break-after: always;" >
		                		...
		                		<table width="100%" style="border-collapse: collapse; border: 0px; padding-bottom: 15px;">
		                			...
		                			<tr>
		                				...
		                				<td width="55%"	style="padding-left: 10px;">'.$profiles->address.'</td>
		                			</tr>
		                			<tr>
		                			...
		                			<tr>
		                				...
		                				<td width="1%" style="font-weight: bold; padding: 5px 0;" align="right">:</td>
		                				<td width="55%"	style="padding-left: 10px;">'.$profiles->phone.'</td>
		                			</tr>
		                			<tr>
		                				<td width="29%" style="font-weight: bold; padding: 5px 0;" >EMAIL ADDRESS</td>
		                				<td width="1%" style="font-weight: bold; padding: 5px 0;" align="right">:</td>
		                				<td width="55%"	style="padding-left: 10px;">'.$users->email.'</td>
		                			</tr>
		                			<tr>
    ...
    //this is the controller for the button that makes the html into a PDF for: Curriculum_Vitae
	function applicantsPDF(){
		$pdf = \App::make('dompdf.wrapper');
		$pdf->loadHTML($this->applicants_convert_to_html());
		return $pdf->stream();
		}

	//this is the html content of the PDF generated for: Narrtive_Report
	function applicants_convert_to_html(){
